...
impl InstructionDisplay for LDType {
    fn to_instr_string(&self, bytes: &[u8]) -> String {
        match self {
            LDType::ByteImm(r8) => format!("{r8}, {:02x}", bytes[1]),
            LDType::WordImm(word) => format!("{word}, {:04x}", word_arg(bytes)),
            LDType::MoveByte(dest, src) => format!("{dest}, {src}"),
            LDType::IndFromA(ind) => format!("{ind}, A"),
...
impl InstructionDisplay for OP {
    fn to_instr_string(&self, bytes: &[u8]) -> String {
        match self {
            ...
            OP::RST(v) => format!("RST {v:#02x}"),
            OP::JR(condition) => format!("JR{condition} {:02x}", bytes[1]),
            OP::JP(condition) => format!("JR{condition} {:04X}", word_arg(bytes)),
            OP::CALL(condition) => format!("CALL{condition} {:04X}", word_arg(bytes)),
            OP::RET(condition) => {
